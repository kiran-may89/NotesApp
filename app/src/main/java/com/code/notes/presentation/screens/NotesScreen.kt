package com.code.notes.ui.screens
...
import com.code.notes.presentation.notes.NotesViewModel
import com.code.notes.presentation.util.Routes
import com.code.notes.ui.components.NoteItem
import com.code.notes.ui.components.OrderSelection
...
fun notesScreen(navController: NavController, viewModel: NotesViewModel = hiltViewModel()) {
    ...
    Scaffold(
        floatingActionButton = {
            FloatingActionButton(
                onClick = {
                    navController.navigate(Routes.AddEditNoteScreen.route)
                },
        ...
        Column(
            ...
            AnimatedVisibility(
                visible = state.orderSectionVisible,
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                items(state.notes) { note ->
                    NoteItem(note = note, modifier = Modifier
                        .fillMaxWidth()
                        .clickable {
                            navController.navigate(Routes.AddEditNoteScreen.route + "?noteId=${note.id}&noteColor=${note.color}")
                        }, onDeleteClick = {
